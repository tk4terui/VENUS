program gpm2gmt
  ...
  if(trim(findex) == "89GV") then 
    iscale = 0.1
    ioffset= 0.0 
    iundef = (/ -8888 , -9999 /)
  elseif(trim(findex) == "89HV") then 
    iscale = 0.1
    ioffset= 0.0 
    iundef = (/ -8888, -9999 /)
  elseif(trim(findex) == "GPM") then 
    iscale = 1.0
    ioffset= 0.0 
    iundef = (/ -8888, -9999 /)
  endif
  ofile = trim(tmpdir)//trim(findex)//"_"//datepole//".txt"  
  open(ounit,file=trim(ofile),action='write',form='formatted')
  fname = trim(findex)//"/"//datepole(1:6)//"/GW1AM2"//datepole(1:8)//"A_110TB"//trim(findex)//datepole(9:10)//".dat"  
