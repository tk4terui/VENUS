...
case $var in
  MAP)
    GMT pscoast -R$Region -J$proJ -W0.01/black -Df -K -O >> $EPS
    cat $MAPTEXT |\
    awk -F, '{print $1, $2}' |\
  ...
  PRMSL)
    ...
    echo ;;
  WIND)
    GMT grdvector $NCORG1 $NCORG2 -R$Region -J$proJ -I$Interval -W0.5,black -Gdarkgray -Q0.02c/0.1c/0.1c -Sl0.2c -C$Color -K -O >> $EPS
    echo ;;
  TMP)
...
if [ "$var" = "WAVE" ] ; then
  rm $NCVAR1 $NCVAR2 $TXTVARS $TXTVAR1 $TXTVAR2 $TXTVAR
fi
