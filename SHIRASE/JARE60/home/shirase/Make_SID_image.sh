#!/bin/bash
now=`date`
echo $now "Maiking SID images from AMSR2 data"
idir0="/home/shirase/Data/AMSR2"
idir1="/home/shirase/Data/"
ifile="${idir0}/%V/%Y%m/GW1AM2%Y%m%d_000%V%P.dat"
vars=("DRLON" "DRLAT")
argvar=SID
ofile="/var/www/shirase/data/%Y%m/AM2SI%Y%m%d%R_SID%S_%P.png"
yymmdds=20181001
yymmdde=100000000
poleALL="SP"
adALL="A"
  ...
  while [ $yymmdd -le $yymmdde ]; do
    ...
    for ad in $adALL; do
      ...
      for shot in $shotALL; do
        ...
        flg=$(( $flg0 * $flg1 ))
        if [ $flg -eq 1 ]; then
          ${script} $yymmdd$ad $pole $argvar $shot
          mv -f ${file1
        fi
      done
    done
